...
------------------------------------------------------------------------------------------------------------
LOAD DATA OVERWRITE `${project_id}.${bigquery_data_beans_curated_dataset}.load_event`                          FROM FILES ( format = 'AVRO', enable_logical_types = false, uris = ['gs://data-analytics-golden-demo/data-beans/v1/export/event/*.avro']);

CREATE TABLE `${project_id}.${bigquery_data_beans_curated_dataset}.event` 
CLUSTER BY (event_id)
AS
SELECT event_id, city_id, event_title, DATE_FROM_UNIX_DATE(event_date) AS event_date, event_time_string, event_venue, event_venue_link, event_address, event_description, event_reviews, event_thumbnail 
  FROM `${project_id}.${bigquery_data_beans_curated_dataset}.load_event` ;

DROP TABLE `${project_id}.${bigquery_data_beans_curated_dataset}.load_event`;
